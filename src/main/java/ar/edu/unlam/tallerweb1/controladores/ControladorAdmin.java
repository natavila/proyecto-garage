...
public class ControladorAdmin {
	...
	@Autowired
	public ControladorAdmin(ServicioRegistro servicioRegistro, ServicioLocalidad servicioLoc,
			ServicioEstacionamiento servicioEst, ServicioGarage servicioGarage, ServicioCliente servicioCliente,
			ServicioPlan servicioPlan, ServicioLogin servicioLogin, ServicioAuto servicioAuto) {
	...
	public ModelAndView administrarGarage(HttpServletRequest request, @PathVariable("id") Long garage1) {
		String rol = (String) request.getSession().getAttribute("roll");
		Long idUsuario = (Long) request.getSession().getAttribute("id");
		Cliente cliente = servicioCliente.consultarClientePorId(idUsuario);
		ModelMap modelo = new ModelMap();
		...
		if (cliente != null)
			if (rol.equals("admin")) {
				...
				if (servicioGarage.cantidadDeLugarEnEst(garage) <= 5
						&& servicioGarage.cantidadDeLugarEnEst(garage) >= 1) {
	...
	public ModelAndView SacarAutoDeGaragePorTicket(HttpServletRequest request, @PathVariable("id") Long id,
	...
	public ModelAndView MostrarHistoricoDeGarage(@PathVariable("id") Long id, HttpServletRequest request) {
	...
	public ModelAndView AutosDeClientes(@ModelAttribute("usuario") Cliente cliente, @PathVariable("id") Long id,
	...
	public String agregarGarage(HttpServletRequest request, @ModelAttribute("garage") Garage garage1) {
		String rol = (String) request.getSession().getAttribute("roll");
		if (rol != null)
			if (rol.equals("admin")) {
